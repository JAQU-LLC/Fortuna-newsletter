...
export function usePosts(published?: boolean) {
  ...
  return useQuery<ListPostsResponse, Error>({
    queryKey: published !== undefined 
      ? ['posts', 'published', published] 
      : ['posts'], // Use 'posts' for admin to match optimistic update query keys
    queryFn: async () => {
...
export function useUpdatePost() {
  ...
  return useMutation<
    ...
    { previousPostsList: ListPostsResponse | undefined; previousPost: Post | undefined }
  >({
    ...
    onMutate: async ({ id, data }) => {
      ...
      const previousPostsList = queryClient.getQueryData<ListPostsResponse>(['posts']);
