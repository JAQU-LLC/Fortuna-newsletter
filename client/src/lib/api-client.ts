...
async function refreshAccessToken(): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      log.error('Token refresh failed', new Error(errorText), { status: response.status });
      return null;
    }
...
async function apiFetch(
  ...
  log.debug('Request', { endpoint, method: options.method || 'GET', hasToken: !!accessToken });
...
export async function loginApi(username: string, password: string) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      log.error('Login failed', new Error(errorData.detail || 'Login failed'), { status: response.status });
      throw new Error(errorData.detail || errorData.message || 'Login failed');
    }

    const data = await response.json();
    
    // Transform role from backend string to UserRole enum
    const { UserRole, isUserRole } = await import('@/models/user');
...
export async function getCurrentUserApi() {
  try {
    ...
    if (!response.ok) {
      ...
      log.error('Get current user failed', new Error(errorText), { status: response.status });
...
export async function listSubscribersApi(
  ...
  return {
    ...data,
    subscribers: data.subscribers.map((sub: any) => transformSubscriber(sub)) as Subscriber[],
  };
}

export async function createSubscriberApi(email: string, name: string): Promise<Subscriber> {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({ detail: 'Failed to create subscriber' }));
...
export async function updateSubscriberApi(
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({ detail: 'Failed to update subscriber' }));
...
export async function createSubscriptionApi(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      log.error('Subscription failed', new Error(errorData.detail || 'Failed to create subscription'), { 
        status: response.status,
        plan: data.plan_id 
      });
      throw new Error(errorData.detail || errorData.message || 'Failed to create subscription');
    }
    ...
    log.info('Subscription created', { plan: responseData.plan, email: data.email });
    return transformed;
  } catch (error) {
...
export async function listPostsApi(params?: ListPostsParams): Promise<ListPostsResponse> {
  const queryParams = new URLSearchParams();
  if (params?.skip !== undefined) queryParams.append('skip', params.skip.toString());
  if (params?.limit !== undefined) queryParams.append('limit', params.limit.toString());
  ...
  try {
    ...
    if (!response.ok) {
      ...
      log.error('List posts failed', new Error(errorText), { status: response.status, params });
    ...
    log.debug('Posts fetched', { total: transformed.total, count: transformed.posts.length, published: params?.published });
    return transformed;
  } catch (error) {
...
export async function updatePostApi(id: string, data: UpdatePostData): Promise<Post> {
  const response = await apiFetch(`/posts/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(data),
  });
